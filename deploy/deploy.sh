...
gcloud run deploy "$COACH_SERVICE" \
  ...
  --cpu=1 \
  --timeout=120 \
  --set-env-vars="RELINK_LLM_PROVIDER=ollama,RELINK_LLM_FALLBACK=vertex,RELINK_ALLOW_MOCK_FALLBACK=0,OLLAMA_API_BASE=https://ollama.com/v1,RELINK_MODEL_COACH=glm-5.2,RELINK_MODEL_STRUCT=kimi-k2.7-code,RELINK_GEMINI_MODEL=gemini-2.0-flash,RELINK_OLLAMA_TIMEOUT=45,GOOGLE_CLOUD_PROJECT=${PROJECT},GOOGLE_CLOUD_LOCATION=${REGION}" \
